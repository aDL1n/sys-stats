...
use windows::Win32::Graphics::Direct2D::Common::{D2D1_ALPHA_MODE_PREMULTIPLIED, D2D1_COLOR_F, D2D1_PIXEL_FORMAT, D2D_SIZE_U};
...
impl D2DRenderer {
    ...
    unsafe fn get_target(&mut self, hwnd: HWND, width: u32, height: u32) -> &ID2D1HwndRenderTarget {
...
pub unsafe fn draw_window(hwnd: HWND) {
    ...
    RENDERER.with(|renderer| {
        ...
        let render_target = renderer.get_target(hwnd, width, height);
        ...
        if let Some(mutex) = MONITOR_STORE.get()
            && let Ok(store) = mutex.lock()
            && let Some(widgets) = WIDGET_STORE.get()
        {
            widgets.draw_all(render_target, &write_factory, &store);
        }
